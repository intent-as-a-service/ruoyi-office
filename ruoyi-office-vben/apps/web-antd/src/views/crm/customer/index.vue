...
// ---------- 内嵌 AI 意图面板：注册页面上下文，供全局悬浮 AI 图标使用 ----------
const visibleRows = ref<CrmCustomerApi.Customer[]>([]);

// 上下文来自页面可见数据（第一行），确保面板与页面数据一致
registerIntentPageContext('crm/customer', () => {
  const row = visibleRows.value[0];
  return row ? { customerId: String(row.id), customerName: row.name } : {};
});
